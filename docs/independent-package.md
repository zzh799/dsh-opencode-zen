...
One mount serves two LLM routes, `opencode-zen` and `opencode-go`: the OpenCode platform's pay-as-you-go plan and its Go subscription. The route ids, the bundle insert id, the settings namespace, and the package name are unchanged from the Zen-only release; the Go plan's configuration lives in the document's `go` block rather than in a second bundle entry, because DSH binds one settings document to one bundle entry id and renaming that id would orphan every existing configuration. See `docs/go-subscription.md`.
